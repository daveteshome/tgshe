...
const API_BASE = import.meta.env.VITE_API_BASE; // e.g. https://<your-backend>/api
      ...
      <div style={styles.thumbWrap}>
        <img
          ...
          onError={(e) => {
            ...
            const isHttp = typeof p.photoUrl === "string" && /^https?:\/\//i.test(p.photoUrl);
            console.warn("[ProductCard:image:error]", {
              productId: p.id,
              tried: imgSrc,
              fallback: isHttp ? p.photoUrl : null,
            });
            if (isHttp) {
              el.src = p.photoUrl!;
            } else {
          ...
          onLoad={() => {
            console.log("[ProductCard:image:ok]", { productId: p.id, src: imgSrc });
          }}
        />
